    ...
    if (document.getElementById('light-chart') && document.getElementById('tooltip-light')) {
        ...
        d3.csv("/historical_data.csv").then(data => {
            ...
            data.forEach(d => {
                ...
                d.Tot_PAR = +d.Tot_PAR; // Convert Tot_PAR to number
            });

            // Group data by date and calculate average Tot_PAR and standard deviation (error)
            const dailyAverage = d3.rollup(data,
                v => {
                    const avg = d3.mean(v, d => d.Tot_PAR);
                    const error = d3.deviation(v, d => d.Tot_PAR); // Calculate standard deviation (error)
            ...
            function applyFilter(filterOption) {
                ...
                if (filterOption === "monthly-light") {
                    ...
                    console.log("Filtered Data (Weekly-light):", filteredData);
                }
            ...
            function updateChart(data) {
                // Remove old elements (path-light, dots, error bars, etc.)
                ...
                if (timeRange > 8 * 24 * 60 * 60 * 1000) {
                    ...
                    groupedData = d3.rollup(data,
                        v => {
                            const avg = d3.mean(v, d => d.avg); // Use 'avg' from grouped data, not Tot_PAR
                ...
                // Sort the array by date to ensure correct path-light drawing
                ...
                console.log("y-axis data:", groupedArray.map(d => d.avg)); // Logs the average Tot_PAR values being passed to y
                ...
                svg.append("path")
                    ...
                    .attr("d", area)
                    .style("fill", "url(#line-gradient-light)");  // Apply the gradient to the area


                // Append the area path-light with the gradient fill
                svg.append("path")
                    .datum(groupedArray)
                    .attr("class", "area-light")
                    .attr("d", area)
                    .style("fill", "url(#line-gradient-light)");  // Apply the gradient

                // Check if all data for the line is valid before appending path-light
                if (groupedArray.every(d => !isNaN(d.avg))) {
                    // Add the updated path-light (with line connecting points)
                    svg.append("path")
                        .datum(groupedArray)
                        .attr("fill", "none")
                        .attr("stroke", "#F0CD76")
