export function renderHumidChart() {
    if (document.getElementById("humidity-chart") && document.getElementById('tooltip-humid')) {
        ...
        const tooltip = d3.select("#tooltip-humid");
        ...
        d3.csv("/historical_data.csv").then(data => {
            ...
            data.forEach(d => {
                ...
                d.Rhair = +d.Rhair; // Convert Rhair to number
            });

            // Group data by date and calculate average Rhair and standard deviation (error)
            const dailyAverage = d3.rollup(data,
                v => {
                    const avg = d3.mean(v, d => d.Rhair);
                    const error = d3.deviation(v, d => d.Rhair); // Calculate standard deviation (error)
            ...
            d3.select("#filter-option-humid").on("change", () => {
                const filterOption = d3.select("#filter-option-humid").property("value");
                applyFilter(filterOption);
            });

            // Initial chart rendering with default filter
            applyFilter("monthly-humid");

            function applyFilter(filterOption) {
                let filteredData;

                if (filterOption === "monthly-humid") {
                ...
                } else if (filterOption === "weekly-humid") {
                    const latestDate = d3.max(dailyData, d => d.date);
                    ...
                    console.log("Filtered Data (Weekly-humid):", filteredData);
                }
            ...
            function updateChart(data) {
                // Remove old elements (path-humid, dots, error bars, etc.)
                svg.selectAll(".dot-humid").remove();
                svg.selectAll(".error-bar-humid").remove();
                svg.selectAll(".hover-area-humid").remove();
                ...
                if (timeRange > 8 * 24 * 60 * 60 * 1000) {
                    ...
                    groupedData = d3.rollup(data,
                        v => {
                            const avg = d3.mean(v, d => d.avg); // Use 'avg' from grouped data, not Rhair
                ...
                // Sort the array by date to ensure correct path-humid drawing
                ...
                console.log("y-axis data:", groupedArray.map(d => d.avg)); // Logs the average Rhair values being passed to y
                ...
                svg.append("path")
                    .datum(groupedArray)
                    .attr("class", "area-humid")
                    .attr("d", area)
                    .style("fill", "url(#line-gradient-humid)");  // Apply the gradient to the area


                // Append the area path-humid with the gradient fill
                svg.append("path")
                    .datum(groupedArray)
                    .attr("class", "area-humid")
                    .attr("d", area)
                    .style("fill", "url(#line-gradient-humid)");  // Apply the gradient

                // Check if all data for the line is valid before appending path-humid
                if (groupedArray.every(d => !isNaN(d.avg))) {
                    // Add the updated path-humid (with line connecting points)
                    svg.append("path")
                        .datum(groupedArray)
                        .attr("fill", "none")
                        .attr("stroke", "#4785E8")
                    ...
                    console.log("Invalid data for line path-humid");
                }
                ...
                svg.selectAll(".dot")
                    ...
                    .on("mouseover", (event, d) => {
                        ...
                        tooltip.style("display", "block")
                            ...
                            .html(`Date: ${d3.timeFormat("%Y-%m-%d")(d.date)}<br>Humidity: ${d.avg.toFixed(2)}%<br>Error: ±${d.error ? d.error.toFixed(2) : 0}`);
                    })
                    .on("mouseout", () => {
