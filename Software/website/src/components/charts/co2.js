export function renderCo2Chart() {
    ...
    if (document.getElementById("co2-chart") && document.getElementById("tooltip-co2")) {
        ...
        d3.select("#filter-option-co2").on("change", function () {
        ...
        function loadData(filterOption) {
            ...
            const apiUrl = filterOption === "monthly-co2"
            ...
            fetch(apiUrl)
                ...
                .then(jsonData => {
                    ...
                    // Filter out zero values for avg_co2 and format the data
                    const historicalData = jsonData.historical_data.filter(d => d.avg_co2 > 0).map(d => ({
                        date: filterOption === "monthly-co2"
                            ? new Date(d.year, d.month - 1)  // Use the year and month for monthly data
                            : parseDate(d.date),  // Use parsed date for weekly data
                        avg: d.avg_co2
                    }));

                    const predictedData = jsonData.predicted_data.filter(d => d.avg_co2 > 0).map(d => ({
                        date: filterOption === "monthly-co2"
        ...
        function updateChart(historicalData, predictedData, filterOption) {
            ...
            svg.selectAll(".line-co2, .dot-co2, .area-co2").remove();
            ...
            const tickFormat = filterOption === "monthly-co2" ? d3.timeFormat("%b") : d3.timeFormat("%d/%m");
            const tickValues = filterOption === "weekly-co2" ? historicalData.map(d => d.date) : null;
            ...
            const dots = svg.selectAll(".dot-co2")
                ...
                .attr("cy", d => y(d.avg))
                .attr("r", 3)
                .style("fill", d => historicalData.includes(d) ? "#808080" : "#88C47A")
        ...
        // Initial data load (monthly CO2 data)
        loadData("monthly-co2");
    }
}
